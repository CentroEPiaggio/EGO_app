...
public class CardboardOverlayView extends LinearLayout {
    ...
    public void setTopicInformation(String topicName, String messageType) {
        ...
        mLeftView = new CardboardOverlayEyeView(getContext(), attrs, "/camera/image/left/compressed", messageType);
        mLeftView.setLayoutParams(params);
        addView(mLeftView);

        mRightView = new CardboardOverlayEyeView(getContext(), attrs, "/camera/image/right/compressed", messageType);
        mRightView.setLayoutParams(params);
        addView(mRightView);

        // Set some reasonable defaults.

        setDepthOffset(-0.05f);
